...
A = np.zeros([10304, 320])  # int matricea=0
cale = r'm0Poze'
for i in range(1,41):  #matricea de antrenare+poza de antrenare
    caleFolderPers = cale + '\s' + str(i) + '\\'
    for j in range(1,9):
        caleB = caleFolderPers + str(j) + '.pgm'
        print(caleB)

        #citirea pozei ca mat 112x92
        pozaAnt = np.array(cv2.imread(caleB, 0))
...
dictNorma = {'1': 1, '2': 2, 'inf': np.inf, 'cos': np.cos(180)}#dictionar pentru 'valori' norma
...
def NN(A, poza, norma):
    ...
    poz = np.argmin(z)#poza aflata la distanta minima

    pozaG = A[:, poz]
    pozaG = pozaG.reshape(112, 92)
    plt.imshow(pozaG)#poza gasita
    plt.show()
...
if __name__ == "__main__":
    input = input("Pentru a apela functia NN alege o norma:")

    NN(A, pozaS, input)#poza cautata